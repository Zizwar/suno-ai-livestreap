...
class AuthService {
	static async refreshToken(session_id, access_token, clerk_js_version = '4.72.0-snapshot.vc141245') {
	...
	static async touch(session_id, access_token, clerk_js_version = '4.72.0-snapshot.vc141245') {
...
export async function handleRequest(request, env, callback, isRefreshToken = true) {
	if (!isRefreshToken) {
		return callback({});
	}
...
export function fetchSuno(url, token, body = null, method = 'POST') {
...
api.get('/api/feed', async (request, env) => {
	...
	return handleRequest(request, env, async (token) => {
